...
import kotlinx.android.synthetic.main.activity_right_drawer.*
import org.avantari.blurdrawerlayout.dummy.DummyContent

class RightDrawerActivity : AppCompatActivity(), RecyclerViewFragment.OnListFragmentInteractionListener {
    ...
    override fun onOptionsItemSelected(item: android.view.MenuItem): Boolean {
        return if (item.itemId == 16908332) {
            toggle()
            true
        } else {
    ...
    private fun toggle() {
        if (blurDrawerLayout.isDrawerVisible(GravityCompat.END)) {
            blurDrawerLayout.closeDrawer(GravityCompat.END)
        } else {
            blurDrawerLayout.openDrawer(GravityCompat.END)
        }
    }
